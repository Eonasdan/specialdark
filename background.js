!function (global) {
	...
	function toggleClient (tab, skipExclusion) {
		var cb = skipExclusion
			...
			: handleManualToggle;
		chrome.tabs.sendMessage(tab.id, {type: 'com.eonasdan.dte__TOGGLE'}, cb);
	}
	...
	chrome.runtime.onMessage.addListener(
		function (request, sender) {
			...
			var tab = sender.tab;
			if (request.type !== 'com.eonasdan.dte__READY' || !tab) {return;}
			...
			if (!darken) {
				chrome.tabs.sendMessage(
					tab.id, {type: 'com.eonasdan.dte__REMOVE_MEDIA_FILTERS'});
			}
		}
	);
